...
import compileArray from "../compile-type/compile-array";
import compileBoolean from "../compile-type/compile-boolean";
import compileBytes from "../compile-type/compile-bytes";
import compileGeneric from "../compile-type/compile-generic";
import compileLatLng from "../compile-type/compile-lat-lng";
import compileNull from "../compile-type/compile-null";
import compileNumeric from "../compile-type/compile-numeric";
import compileObject from "../compile-type/compile-object";
import compilePath from "../compile-type/compile-path";
import compileString from "../compile-type/compile-string";
import compileTimestamp from "../compile-type/compile-timestamp";
...
describe("type compilers", () => {
	...
	describe("firestore.path", () => {
		it("compiles basic schema", () => {
			assert.equal(
				...
				`((ref is map)&&(ref.keys().hasOnly(["path"])&&(ref.path is path)))`
			);
		});
	});
	...
	describe("array", () => {
		...
		it('supports "additionalItems"', () => {
			assert.equal(
				compileArray(
					{ type: "array", items: [true, { type: "boolean" }], additionalItems: true },
					"ref",
					"s"
				),
				"((((ref is list)&&(ref.size()>=2))&&ref[0])&&(ref[1] is bool))"
	...
	describe("numeric", () => {
		it("compiles basic schema", () => {
			...
			assert.equal(
				compileNumeric({ type: ["integer", "number"] }, "ref"),
				"((ref is int)||(ref is float))"
			);
		});
